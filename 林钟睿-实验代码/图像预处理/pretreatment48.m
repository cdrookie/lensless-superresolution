...
format_choose = 3;
Picture_format = ['.tif';'.jpg';'.bmp'];

x0 = 2764;
y0 = 3872;
K = 48; % 更新为 16*3=48，因为每个通道有 16 幅图像
...
r = NaN(show_size, show_size, 16);
g = NaN(show_size, show_size, 16);
b = NaN(show_size, show_size, 16);
y = NaN(show_size, show_size, K);

hologram_r = NaN(x0, y0, 16);
hologram_g = NaN(x0, y0, 16);
hologram_b = NaN(x0, y0, 16);
zero_padding = zeros(x0, y0);
background_r = NaN(x0, y0, 16);
background_g = NaN(x0, y0, 16);
background_b = NaN(x0, y0, 16);
